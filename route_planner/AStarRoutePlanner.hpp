 ...
 * 使用A*启发式搜索算法，实现了路径规划
 */
class AStarRoutePlanner : public RoutePlanner {
...
public:
    explicit AStarRoutePlanner(std::shared_ptr<MarkMap> obstacleMap) : RoutePlanner(std::move(obstacleMap)) {
    }
    ...
    Path plan(const Point &start, const Point &end) override {
        ...
        while (!frontier.empty()) {   //只要优先队列不为空
            ...
            for (auto &next:neighbors) {    //获取所有可以走的路线
                ...
                int new_cost = cost_so_far[current] + deltaCost;    //每走一格，当前代价+

                if (cost_so_far.count(next) == 0 || new_cost < cost_so_far[next]) {  //如果代价未计算或新代价较小
                    cost_so_far[next] = new_cost;   //更新代价
                    int priority = new_cost + next.getHeuristic(end);   //计算总代价=当前代价+预估代价
